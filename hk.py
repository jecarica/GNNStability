# hk_stability_imdb_plus.py
# Hiraoka–Kusano–inspired IMDB-BINARY training with stability regularization
...
def stratified_split(dataset, per_class_train=400, seed=17):
    # Assumes binary IMDB-BINARY labels {0,1}
    set_seed(seed)
    idx0 = [i for i, d in enumerate(dataset) if int(d.y.item()) == 0]
    idx1 = [i for i, d in enumerate(dataset) if int(d.y.item()) == 1]
    np.random.shuffle(idx0); np.random.shuffle(idx1)
    train_idx = idx0[:per_class_train] + idx1[:per_class_train]
    test_idx  = idx0[per_class_train:] + idx1[per_class_train:]
    train_idx.sort(); test_idx.sort()
    train_ds = dataset.__class__(dataset.root, dataset.name)  # reload raw
    test_ds  = dataset.__class__(dataset.root, dataset.name)
    # slice into new InMemoryDatasets
    train_ds.data, train_ds.slices = dataset.data, dataset.slices
    test_ds.data,  test_ds.slices  = dataset.data, dataset.slices
    # but keep only selected indices by simple list
...
if __name__ == "__main__":
    ...
    ds_full = HKIMDBDataset(
        root="data/ENZYMES", name="ENZYMES",
        r0=0.4, r1=1.2, res=10, sigma=0.1,
        max_deg=50, p_edge_pert=0.05
    )

    # Stratified split (400/400 per class for train, remaining ~100/100 test)
    train_ds, test_ds = stratified_split(ds_full, per_class_train=400, seed=SEED)
    train_loader = DataLoader(train_ds, batch_size=64, shuffle=True)
    test_loader  = DataLoader(test_ds, batch_size=64)

    topo_dim = train_ds[0].topo.numel()     # = 2*(res^2) = 200 if res=10
    in_dim   = train_ds.num_node_features   # one-hot degree (<= max_deg+1)
