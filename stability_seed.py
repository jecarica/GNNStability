...
class TopoGIN(nn.Module):
    ...
    def _reshape_topo(topo_vec, batch, topo_dim):
        ...
        flat = topo_vec.reshape(-1)
        need = B * topo_dim
        if flat.numel() < need:
            flat = torch.cat([flat, flat.new_zeros(need - flat.numel())], dim=0)
        return flat[:need].view(B, topo_dim)
...
# ----------------------------------
def _reshape_topo_any(t, batch, topo_dim):
    # utility used in losses/eval
    B = int(batch.max().item()) + 1
...
def stability_regularizer(logits_orig, logits_pert, topo, topo_pert, batch, topo_dim,
                          lam_pred=1.0, lam_pi=0.05):
    ...
    topo = _reshape_topo_any(topo, batch, topo_dim)
    topo_pert = _reshape_topo_any(topo_pert, batch, topo_dim)
...
def train_epoch(model, loader, opt, device, topo_dim):
    ...
    for data in loader:
        ...
        # logits drift + small PI drift
        stab = stability_regularizer(out, out_pert, data.topo, data.topo_pert,
                                     data.batch, topo_dim, lam_pred=1.0, lam_pi=0.05)
        loss = ce + 0.3 * stab
...
@torch.no_grad()
def eval_cert(model, loader, device, topo_dim):
    model.eval()
    vals = []
    for data in loader:
        data = data.to(device)
        ...
        topo = _reshape_topo_any(data.topo, data.batch, topo_dim)
        topo_pert = _reshape_topo_any(data.topo_pert, data.batch, topo_dim)
...
if __name__ == "__main__":
    ...
    topo_dim = ds[0].topo.numel()        # = 2*(res^2) = 200
    in_dim   = ds.num_node_features      # e.g., 51
    ...
    N = len(ds)
    labels = torch.tensor([ds[i].y.item() for i in range(N)])
    classes = labels.unique().tolist()
    idx_per_class = {c: (labels == c).nonzero(as_tuple=True)[0] for c in classes}

    train_idx_parts, test_idx_parts = [], []
    for c in classes:
        idx = idx_per_class[c]
        idx = idx[torch.randperm(idx.numel(), generator=g)]
        n_train = int(0.8 * idx.numel())
    ...
    # sanity print counts
    def count_by_class(idx):
        y = labels[idx]
        return {int(c): int((y == c).sum()) for c in classes}
    print("Stratified split:",
          "train per class =", count_by_class(train_idx),
          "test per class =", count_by_class(test_idx))
    # --------------------------------
    ...
    for epoch in range(1, 51):
        ...
        if epoch % 5 == 0:
            ...
            cert  = eval_cert(model, test_loader, device, topo_dim)
            best  = max(best, clean)
            print(f"Epoch {epoch:03d} | Loss {loss:.4f} | Clean {clean:.3f} | "
                  f"Noisy {noisy:.3f} | Drop {clean-noisy:.3f} | CertRadius~ {cert:.4f}")
    ...
    final_cert  = eval_cert(model, test_loader, device, topo_dim)
